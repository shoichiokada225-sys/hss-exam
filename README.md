...
- [ ] テスト受験を1回通し（本試験フローで）→シート3タブに行が入ることを確認→**その行を手で削除**（`purgeTestRows`は `test_conn_`/`sample_` で始まるIDしか消さない。通しテストの行のIDは `exam_氏名_…` なので対象外）
- [ ] 受験者へ「前日までに電波のある場所でアプリを一度開いて閉じる」を5言語で案内（オフライン開始の生命線）
- [ ] 本試験パスワードの伝達方法を決める（推奨: 開始直前に口頭＋板書、終了後に消す。事前配布しない）。**現在のconfig.jsのハッシュは本READMEの「デフォルトパスワード hss2024」とは別物**＝実際のパスワードを管理者が把握していることを前日までに確認する
- [ ] 受験者名簿の正式表記一覧を印刷（会場掲示用。氏名の表記ゆれが突合の最大の敵）

#### 当日朝
- [ ] 管理者端末でアプリ起動→ホーム表示・言語切替・デモ1問だけ動作確認
- [ ] **開始直前と終了10分前に** ブラウザで `/exec`（config.jsのwebhookUrl）を1回開いてGASを温める（放置後の初回応答は実測9〜19秒。2026-09-10の修正でクライアント側も25秒まで待つが、温めておくと最初の終了組が速く✅になる）
...
## 2026-09-10 100人同時受験に向けた実測と修正
- 実測: GAS `/exec` は放置後の初回応答 9〜19秒、30人一斉POSTで中央値28秒・最大191秒、100人（時間切れ組180秒分散を再現）で**サーバー記録は100/100**だが、旧クライアント（verify待ち6秒）は37人が再試行・1人が⚠表示になった（原因の大半はverifyの6秒タイムアウト）
- 修正: verify/probe待ち 6→25秒、再試行 6→12回、バックオフ上限 8→20秒、probe失敗時も記録確認してから⚠、デモ終了で中断中の本試験の進行を消さない、localStorage例外（iOS「すべてのCookieをブロック」）で初期化が止まらない、送信中は「ホームに戻る」を隠す。sw.js v8
- 検証: tests/e2e-review-flag.mjs 46件 + 本試験フローE2E（モックGAS: 正常/遅延12秒/busy×2/停止→再起動再送/verify不能/中断再開/デモ後の進行保持/localStorage例外/送信中ホーム非表示）32件 全PASS
